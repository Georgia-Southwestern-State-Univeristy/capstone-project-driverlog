...
function DriverBadge({ value }) {
  const v = String(value || "Unknown driver");
...
// ── DRIVER VIEW ──────────────────────────────────────────────
function DriverView({ allEntries, status, error, onRefresh }) {
  const openCount = allEntries.filter((e) => e.status !== "Resolved").length;
  const resolvedCount = allEntries.filter((e) => e.status === "Resolved").length;
    ...
    <div className="relative mx-auto max-w-4xl w-full px-4 py-8 space-y-6">
      ...
      <div className="rounded-3xl p-6 sm:p-8 shadow-sm" style={{ border: "1px solid rgba(199,183,136,0.25)", background: "rgba(255,255,255,0.05)", backdropFilter: "blur(8px)" }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white">My Flags</h1>
            <p className="mt-1 text-sm" style={{ color: "#C7B788" }}>Flagged activity on your routes. Contact your manager to resolve these.</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold" style={{ background: "rgba(199,183,136,0.12)", color: "#C7B788", border: "1px solid rgba(199,183,136,0.25)" }}>Status: {status}</span>
              <span className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold" style={{ background: "rgba(200,60,60,0.12)", color: "#f87171", border: "1px solid rgba(200,60,60,0.25)" }}>{openCount} open</span>
              <span className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold" style={{ background: "rgba(102,175,182,0.12)", color: "#66AFB6", border: "1px solid rgba(102,175,182,0.25)" }}>{resolvedCount} resolved</span>
            </div>
          ...
          <button onClick={onRefresh} className="rounded-xl px-4 py-2 text-sm font-semibold text-white self-start" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(199,183,136,0.3)" }}>Refresh</button>
        </div>
      </div>
      ...
      <div className="rounded-3xl shadow-sm overflow-hidden" style={{ border: "1px solid rgba(199,183,136,0.2)", background: "rgba(255,255,255,0.04)", backdropFilter: "blur(8px)" }}>
        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(199,183,136,0.15)", background: "rgba(0,0,0,0.1)" }}>
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: "#C7B788" }}>Sorted by severity — High flags need attention first.</p>
          </div>
          <div className="hidden sm:flex items-center gap-2">
            {Object.entries(severityStyles).map(([label, s]) => (
              <span key={label} className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>{label}</span>
            ))}
          </div>
        </div>

        <div className="divide-y" style={{ borderColor: "rgba(199,183,136,0.08)" }}>
          {sorted.map((entry) => (
            <div key={entry.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 hover:bg-white/5">
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <SeverityBadge severity={entry.severity} />
                  {entry.status === "Resolved" && (
                    <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style={{ background: "rgba(102,175,182,0.15)", color: "#66AFB6", border: "1px solid rgba(102,175,182,0.3)" }}>✓ Resolved</span>
                  )}
                </div>
                <p className="text-sm font-semibold text-white">{entry.reason}</p>
                {entry.notes && <p className="text-xs" style={{ color: "#C7B788" }}>{entry.notes}</p>}
              </div>
              <p className="text-xs shrink-0" style={{ color: "#C7B788" }}>
                {entry.createdAt ? new Date(entry.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "-"}
              </p>
            </div>
          ))}
          {allEntries.length === 0 && status !== "Loading..." && (
            <div className="px-6 py-10 text-sm" style={{ color: "#C7B788" }}>
              {status === "Connected" ? "No flags on your account 🎉" : "Loading..."}
            </div>
          )}
        </div>

        <div className="px-6 py-3" style={{ borderTop: "1px solid rgba(199,183,136,0.12)", background: "rgba(0,0,0,0.1)" }}>
          <p className="text-xs" style={{ color: "#C7B788" }}>Flags are created automatically by the system. Contact your manager to resolve them.</p>
        </div>
      </div>
    </div>
  );
...
// ── ADMIN / MANAGER VIEW ─────────────────────────────────────
function AdminManagerView({ allEntries, status, error, onRefresh }) {
          ...
          <div className="w-full max-w-lg rounded-3xl p-6 shadow-xl space-y-4" style={{ background: "#1e2a2b", border: "1px solid rgba(199,183,136,0.25)" }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-1">
                  <DriverBadge value={selectedEntry.driverEmail || selectedEntry.driverId || "Unknown driver"} />
                </div>
              </div>
      ...
      <div className="rounded-3xl p-6 sm:p-8 shadow-sm" style={{ border: "1px solid rgba(199,183,136,0.25)", background: "rgba(255,255,255,0.05)", backdropFilter: "blur(8px)" }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={onRefresh} className="rounded-xl px-4 py-2 text-sm font-semibold text-white" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(199,183,136,0.3)" }}>Refresh</button>
      ...
      <div className="rounded-3xl shadow-sm overflow-hidden" style={{ border: "1px solid rgba(199,183,136,0.2)", background: "rgba(255,255,255,0.04)", backdropFilter: "blur(8px)" }}>
        ...
        <div className="overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(showAllEntries ? flaggedEntries : flaggedEntries.slice(0, 20)).map((entry) => (
                <tr key={entry.id} className="hover:bg-white/5" style={{ borderBottom: "1px solid rgba(199,183,136,0.08)" }}>
                  <td className="px-6 py-4">
                    <DriverBadge value={entry.driverEmail || entry.driverId || "Unknown driver"} />
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => setSelectedEntry(entry)} className="rounded-xl px-3 py-1.5 text-xs font-semibold" style={{ background: "rgba(102,175,182,0.2)", border: "1px solid rgba(102,175,182,0.4)", color: "#66AFB6" }}>Review</button>
                  </td>
                </tr>
              ))}
              {flaggedEntries.length === 0 && status !== "Loading..." && (
                <tr>
                  <td className="px-6 py-10" style={{ color: "#C7B788" }} colSpan={5}>
                    {showHistory ? "No resolved entries yet." : status === "Connected" ? "No flagged entries 🎉" : "Loading..."}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2" style={{ borderTop: "1px solid rgba(199,183,136,0.12)", background: "rgba(0,0,0,0.1)" }}>
          <p className="text-xs" style={{ color: "#C7B788" }}>
            {flaggedEntries.length > 20
              ? showAllEntries
                ? `Showing all ${flaggedEntries.length} entries`
                : `Showing 20 of ${flaggedEntries.length} entries`
              : showHistory
              ? "Showing resolved entries only."
              : "Tip: Review High severity entries before exporting."}
          </p>
          <div className="flex items-center gap-4">
            {flaggedEntries.length > 20 && (
              <button onClick={() => setShowAllEntries(!showAllEntries)} className="text-sm font-semibold" style={{ color: "#9FCC81" }}>
                {showAllEntries ? "Show less ↑" : `View all (${flaggedEntries.length}) →`}
              </button>
            )}
            <button onClick={() => { setShowHistory(!showHistory); setShowAllEntries(false); }} className="text-xs font-semibold" style={{ color: "#9FCC81" }}>
...
export default function Flagged() {
  ...
  async function loadFlaggedEntries() {
    try {
      setError(null);
      setStatus("Loading...");
      const [data, meData] = await Promise.all([getFlaggedEntries(), getMe()]);
  ...
  return (
    <div className="relative overflow-x-hidden">
      ...
      {isDriver
        ...
        : <AdminManagerView allEntries={allEntries} status={status} error={error} onRefresh={loadFlaggedEntries} />
      }
    </div>
  );
}